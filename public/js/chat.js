...
function onLoad() {
  ...
  const urlParams = new URLSearchParams(window.location.search);// pega os parametros da url

  // separa os parametros da url
  const name = urlParams.get("name");
  const avatar = urlParams.get("avatar");
  const email = urlParams.get("email");
  

  // adicionar image e nome do usuario logado
  ...
  // evento que fica escutando se um usuario se conectou
  socket.emit("start", {
    email,
    name,
    avatar,
  ...
  // evento de novo usuario dentro da sala
  ...
  // envia um evento (get_users) para o servidor
  socket.emit("get_users", {
    email,
    name,
    avatar,
  }, (users) => {
    // recebe os usuarios da sala e exibe na tela
    users.map((user) => {
      // se o usuario for diferente do usuario logado, adiciona na lista
      if (user.email !== email) {
        addUser(user);
      }
    });
  ...
  // ouve um evento (message) que recebe uma nova mensagem
  socket.on("message", (data) => {
    // confere o id da sala
    if (data.message.roomId === idChatRoom) {
      // adiciona a mensagem na tela do chat
      addMessage(data);
    }
  });
...
// adicionar o icone de usuario na sala
